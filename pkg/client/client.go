...
func (local *LsLocal) Listen(didListen func(listenAddr *net.TCPAddr)) error {
	return cipher.ListenEncryptedTCP(local.ListenAddr, local.Cipher, local.handleConn, didListen)
}

func (local *LsLocal) handleConn(userConn *cipher.SecureTCPConn) {
	defer userConn.Close()

	proxyServer, err := cipher.DialEncryptedTCP(local.RemoteAddr, local.Cipher)
	if err != nil {
		log.Println(err)
		return
	}
